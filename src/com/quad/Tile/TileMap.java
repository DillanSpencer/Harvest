...
public class TileMap {
	...
	public void loadGameTiles() {
		...
		for(int row = 0; row < numRows; row++) {
			for(int col = 0; col < numCols;col++) {
				...
				if(getIndex(row, col) == 20) {
					t = new Tree(this);
					t.setPosition(col * tileSize + tiles[r][c].x, 
								  row * tileSize + tiles[r][c].y);
					trees.add(t);
				}
			}
		}
	...
	public void setPosition(double x, double y) {
		...
		Tween.to(this, PlayerAccessor.POSITION_Y, 0.001f).target((float)y, (float)this.y).start(tween);
		}
		else y = 0;
		if(x < 0 && x > (-width + gc.getWidth())) {
			Tween.to(this, PlayerAccessor.POSITION_X, 0.001f).target((float)x, (float)this.x).start(tween);
		}
		else x = 0;
